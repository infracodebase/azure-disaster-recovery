...
<#
.SYNOPSIS
    Restores networking configuration of VMs after Site Recovery failover including ASG, NIC, and Application Gateway configurations.

.DESCRIPTION
    This script compares current networking configurations with saved pre-failover configurations and applies
    any missing configurations. It restores Application Security Group memberships, static IP assignments,
...
.EXAMPLE
    .\Restore-NetworkingConfig.ps1 -MasterConfigFile "C:\ASR\Configs\MASTER-networking-config-20241201-143022.json" -ResourceGroupName "webapp-prod-secondary-rg" -DryRun

.EXAMPLE
    .\Restore-NetworkingConfig.ps1 -ConfigurationPath "." -ResourceGroupName "webapp-prod-secondary-rg" -VmNames @("web-vm-1", "web-vm-2") -Force
...
.NOTES
    ...
    This script is designed to run as part of Azure Site Recovery automation post-failover.
    Ensure the execution account has appropriate permissions to modify networking configurations.
#>
        ...
        foreach ($lbPoolMembership in $nicConfig.LoadBalancerBackendPools) {
            ...
            if ($currentIpConfig) {
                ...
                if (-not $currentLbPool) {
                    # Try to find the load balancer in the target resource group
                    $targetLbName = $lbName -replace "-primary-", "-secondary-"
...
function Compare-ApplicationGatewayBackendPools {
    param(
        [Parameter(Mandatory = $true)]
        [object]$SavedConfig,
    ...
    foreach ($nicConfig in $SavedConfig.NetworkInterfaces) {
        ...
        foreach ($appGwPoolMembership in $nicConfig.ApplicationGatewayBackendPools) {
            $ipConfigName = $appGwPoolMembership.IpConfigurationName
            $appGwName = $appGwPoolMembership.ApplicationGatewayName
            $poolName = $appGwPoolMembership.BackendPoolName

            # Check if this App Gateway backend pool membership exists in current configuration
            $currentIpConfig = $CurrentNic.IpConfigurations | Where-Object { $_.Name -eq $ipConfigName }
            if ($currentIpConfig) {
                $currentAppGwPool = $currentIpConfig.ApplicationGatewayBackendAddressPools | Where-Object { $_.Name -eq $poolName }
                if (-not $currentAppGwPool) {
                    # Try to find the application gateway in the target resource group
                    $targetAppGwName = $appGwName -replace "-primary-", "-secondary-"

                    $differences += @{
                        Type = "ApplicationGatewayBackendPool"
                        Action = "Add"
                        VmName = $VmName
                        NicName = $CurrentNic.Name
                        IpConfigName = $ipConfigName
                        ApplicationGatewayName = $targetAppGwName
                        ApplicationGatewayResourceGroup = $ResourceGroupName
                        BackendPoolName = $poolName
                        Details = "Missing App Gateway backend pool membership: $poolName in $targetAppGwName"
                    }
                }
            }
        }
...
function Restore-ApplicationSecurityGroupMembership {
    ...
    try {
        ...
        if (-not $asg) {
            # Try to find ASG in the current resource group
...
function Restore-ApplicationGatewayBackendPoolMembership {
    param(
        [Parameter(Mandatory = $true)]
        [object]$Difference
    )
    ...
    try {
        Write-Log "  Restoring App Gateway pool membership: $($Difference.BackendPoolName) for $($Difference.NicName)/$($Difference.IpConfigName)" "ACTION"

        if ($DryRun) {
            Write-Log "    [DRY-RUN] Would add App Gateway backend pool membership" "INFO"
            return $true
        }

        # Get the application gateway
        $appGw = Get-AzApplicationGateway -ResourceGroupName $Difference.ApplicationGatewayResourceGroup -Name $Difference.ApplicationGatewayName -ErrorAction SilentlyContinue
        if (-not $appGw) {
            Write-Log "    Application Gateway $($Difference.ApplicationGatewayName) not found" "WARNING"
            return $false
        }

        # Get the backend pool
        $backendPool = $appGw.BackendAddressPools | Where-Object { $_.Name -eq $Difference.BackendPoolName }
        if (-not $backendPool) {
            Write-Log "    Backend pool $($Difference.BackendPoolName) not found" "WARNING"
            return $false
        }
        ...
        $ipConfig.ApplicationGatewayBackendAddressPools += $backendPool

        # Update the network interface
        Set-AzNetworkInterface -NetworkInterface $nic | Out-Null
        Write-Log "    Successfully added App Gateway backend pool membership" "SUCCESS"
        return $true
    }
    catch {
        Write-Log "    Failed to restore App Gateway backend pool membership: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Restore-NetworkingConfiguration {
    ...
    Write-Log "Restoring configuration for VM: $VmName" "INFO"
    ...
    foreach ($diff in $vmDifferences) {
        ...
        switch ($diff.Type) {
            ...
            "ApplicationGatewayBackendPool" {
                $success = Restore-ApplicationGatewayBackendPoolMembership -Difference $diff
            }
            default {
...
function Main {
    ...
    try {
        ...
        $savedConfigurations = Get-SavedConfigurations -MasterConfigPath $masterConfigPath
            ...
            try {
                ...
                foreach ($nicRef in $vm.NetworkProfile.NetworkInterfaces) {
                    ...
                    if ($currentNic) {
                        ...
                        $vmDifferences += Compare-ApplicationSecurityGroups -SavedConfig $savedConfig -CurrentNic $currentNic -VmName $vmName
                        $vmDifferences += Compare-StaticIpConfigurations -SavedConfig $savedConfig -CurrentNic $currentNic -VmName $vmName
                        $vmDifferences += Compare-LoadBalancerBackendPools -SavedConfig $savedConfig -CurrentNic $currentNic -VmName $vmName
                        $vmDifferences += Compare-ApplicationGatewayBackendPools -SavedConfig $savedConfig -CurrentNic $currentNic -VmName $vmName
                    }
                }
        ...
        foreach ($vmName in $allDifferences.Keys) {
            $result = Restore-NetworkingConfiguration -VmName $vmName -SavedConfig $savedConfigurations[$vmName] -AllDifferences $allDifferences
